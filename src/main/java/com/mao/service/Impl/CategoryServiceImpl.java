...
import com.mao.dao.GoodsCategoryMapper;
...
import com.mao.vo.SearchPageCategoryVO;
import com.mao.vo.SecondLevelCategoryVO;
import com.mao.vo.ThirdLevelCategoryVO;
...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<CategoryVo> getCategoriesForIndex() {
        ...
        if (!CollectionUtils.isEmpty(firstLevelCategories)) {
            ...
            //获取二级分类的数据
            List<GoodsCategory> secondLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(firstLevelCategoryIds, CategoryEnum.LEVEL_TWO.getLevel(), 0);
            if (!CollectionUtils.isEmpty(secondLevelCategories)) {
                List<Long> secondLevelCategoryIds = secondLevelCategories.stream().map(GoodsCategory::getCategoryId).collect(Collectors.toList());
                //获取三级分类的数据
                List<GoodsCategory> thirdLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(secondLevelCategoryIds, CategoryEnum.LEVEL_THREE.getLevel(), 0);
                if (!CollectionUtils.isEmpty(thirdLevelCategories)) {
                    ...
                    if (!CollectionUtils.isEmpty(secondLevelCategoryVOS)) {
                        ...
                        for (GoodsCategory firstCategory : firstLevelCategories) {
                            CategoryVo newBeeMallIndexCategoryVO = new CategoryVo();
    ...
    public SearchPageCategoryVO getCategoriesForSearch(Long categoryId) {
        ...
        if (thirdLevelGoodsCategory != null && thirdLevelGoodsCategory.getCategoryLevel() == CategoryEnum.LEVEL_THREE.getLevel()) {
            //获取当前三级分类的二级分类
            GoodsCategory secondLevelGoodsCategory = goodsCategoryMapper.selectByPrimaryKey(thirdLevelGoodsCategory.getParentId());
            if (secondLevelGoodsCategory != null && secondLevelGoodsCategory.getCategoryLevel() == CategoryEnum.LEVEL_TWO.getLevel()) {
                //获取当前二级分类下的三级分类List
                List<GoodsCategory> thirdLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(Collections.singletonList(secondLevelGoodsCategory.getCategoryId()), CategoryEnum.LEVEL_THREE.getLevel(), Constants.SEARCH_CATEGORY_NUMBER);
